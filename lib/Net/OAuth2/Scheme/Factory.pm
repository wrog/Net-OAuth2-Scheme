...
generate sequential IDs using a counter.  Providing I<tag> sets C<counter_tag> (which see).

=item C<'random'>

=item C<< ['random', I<n> ] >>

generate random IDs using the random number generator.  Providing I<n> sets C<v_id_random_length>.

=back

=item C<v_id_random_length>

integer; for when C<< v_id => 'random' >>, the number of random bytes to
include (must be at least 8).
...
collisions between) IDs generated by multiple hosts for when there are
to be multiple authorization servers issuing tokens for the same
resource.
